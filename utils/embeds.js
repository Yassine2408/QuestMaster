...
function createProfileEmbed(user, playerData) {
    ...
    embed.addField('Level', `${playerData.level} (${playerData.xp}/${helpers.getXpForLevel(playerData.level)} XP)`, true);
    embed.addField('Gold', `${playerData.gold} ${CONFIG.currency}`, true);
    embed.addField('Health', `${playerData.stats.currentHealth}/${playerData.stats.maxHealth}`, true);
    embed.addField('Strength', `${playerData.stats.strength}`, true);
    embed.addField('Defense', `${playerData.stats.defense}`, true);
    ...
    embed.addField('XP Progress', `${xpProgress} (${playerData.xp}/${xpRequired})`, false);
    ...
    embed.addField('Equipped Items', equippedText, false);
    ...
    if (playerData.pet) {
        ...
        embed.addField('Pet Companion', petText, false);
    }
    ...
    if (playerData.buffs) {
        ...
        if (buffsText) {
            embed.addField('Active Buffs', buffsText, false);
        }
    }
    ...
    if (playerData.quests) {
        ...
        embed.addField('Quests', `${activeQuestCount} Active, ${completedQuestCount} Completed`, true);
    }
    
    // Show achievement count
    const achievementCount = playerData.achievements.length;
    embed.addField('Achievements', `${achievementCount} Unlocked`, true);
    ...
    if (playerData.joinedAt) {
        ...
        embed.addField('Player Since', `${joinedDate.toDateString()} (${daysSinceJoined} days)`, false);
    }
    ...
    if (cooldownText) {
        embed.addField('Cooldowns', cooldownText, false);
    }
...
function createHelpEmbed() {
    ...
    embed.addField('🌾 Resource Gathering', 
    ...
    // Adventure section
    embed.addField('⚔️ Adventure', 
        '`!adventure` or `!adv` - List adventure locations\n' +
    ...
    embed.addField('📦 Inventory & Equipment', 
        '`!inventory search <term>` - Search your inventory\n' +
    ...
    embed.addField('👥 Party System', 
        '`!party` - View your party status\n' +
    ...
    embed.addField('🏆 Other Commands', 
        '`!leaderboard` or `!lb` - View player rankings\n' +
...
function createShopEmbed(playerGold) {
    ...
    for (const [category, items] of Object.entries(categories)) {
        if (items.length > 0) {
            embed.addField(
                `${helpers.capitalizeFirst(category)}s`,
                items.join('\n')
            );
        }
    }
...
function createQuestEmbed(quest, progress) {
    ...
    embed.addField('Objective', objectiveText, false);
    
    // Add progress if available
    if (progress !== undefined) {
        const progressBar = helpers.getProgressBar(progress, quest.target, 10);
        embed.addField('Progress', `${progressBar} (${progress}/${quest.target})`, false);
    }
    ...
    embed.addField('Rewards', rewardText, false);
    ...
    embed.addField('Required Level', quest.minLevel.toString(), true);
...
function createPetStatusEmbed(playerData) {
    ...
    embed.addField('Level', 
        `${playerData.petStats.level} (${playerData.petStats.xp}/${petXpRequired} XP)\n` +
        `${petXpProgress}`, 
        false
    );
    ...
    embed.addField('Happiness', `${happinessBar} (${playerData.petStats.happiness}/100)`, true);
    embed.addField('Hunger', `${hungerBar} (${playerData.petStats.hunger}/100)`, true);
    
    // Pet bonuses
    const petBonus = Math.floor(playerData.petStats.level / 2);
    embed.addField('Bonuses', 
        `+${petBonus} Strength\n` +
        `+${petBonus} Defense\n` +
    ...
    if (petData && petData.abilities) {
        ...
        if (unlockedAbilities.length > 0) {
            embed.addField('Abilities', 
                unlockedAbilities.map(a => `${a.name}: ${a.description}`).join('\n'),
                false
            );
        }
        
        if (lockedAbilities.length > 0) {
            embed.addField('Locked Abilities', 
                lockedAbilities.map(a => `${a.name} (Unlocks at Level ${a.level})`).join('\n'),
                false
            );
        }
    }
...
function createNotificationEmbed(playerData) {
    ...
    embed.setDescription(`You have ${playerData.notifications.filter(n => !n.read).length} unread notifications.`);
    ...
    recentNotifications.forEach((notification, index) => {
        ...
        embed.addField(
            `${index + 1}. ${timeAgo}${readStatus}`,
            notification.message
        );
    });
    
    embed.setFooter({ text: 'Notifications are automatically marked as read when viewed' });
